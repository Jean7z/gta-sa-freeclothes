...
- **Shop rows** — the clothing script (`scriptv1.img`) adds a shop row only when its SCM gate flag is `1` (normally set by story purchases). The mod **enables those gates** so the script adds the SHOP2–SHOP7 rows itself with its native index dispatch, and re-forges them whenever they reset: when the safehouse wardrobe opens (via `LoadShop`), on startup/save load (via `CShopping::Load`), and on a new game (via `CShopping::Init`). A savegame stores the vanilla gate values, and a **new game resets them to vanilla right after init** — so v1.4 also runs a per-frame guard (`CPlayerInfo::Process`) that re-forges the gates the moment they drop back to `0`. Without it, a new game would show just Binco until you saved and loaded.
